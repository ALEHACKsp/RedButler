...
const UCHAR InjShellcodeNative[] = {
...
NTSTATUS InjFindProcess(_In_ HANDLE ProcessId, _Out_ PEPROCESS* Process, _Out_ PBOOLEAN IsWow64) {
...
NTSTATUS InjPrepareShellcode(_In_ PUNICODE_STRING ModulePath,
	_In_ PVOID LdrLoadDll, _In_ BOOLEAN IsWow64, _Out_ PVOID* Shellcode, _Out_ PVOID* ShellcodeParam) {
	...
	const UCHAR* ShellcodeBytes = IsWow64 ? InjShellcodeWow64 : InjShellcodeNative;
	const SIZE_T ShellcodeSize = IsWow64 ? sizeof(InjShellcodeWow64) : sizeof(InjShellcodeNative);
...
NTSTATUS InjExecuteShellcode(_In_ PEPROCESS Process,
	_In_ BOOLEAN IsWow64, _In_ PVOID Shellcode, _In_ PVOID ShellcodeParam) {

	return ApcQueueExecution(Process, IsWow64, Shellcode, ShellcodeParam);
}

NTSTATUS InjectDLL(_In_ HANDLE ProcessId, _In_ PUNICODE_STRING ModulePath) {
	PEPROCESS Process = NULL;
	BOOLEAN IsWow64 = FALSE;

	NTSTATUS status = InjFindProcess(ProcessId, &Process, &IsWow64);
	if (!NT_SUCCESS(status)) {
		LogError("InjFindProcess failed with status %08x.", status);
		return status;
	}
	...
	if (LdrLoadDll == NULL) {
		LogError("UtlGetLdrLoadDll failed with status %08x.", status);
	...
	if (!NT_SUCCESS(status)) {
		LogError("InjPrepareShellcode failed with status %08x.", status);
	...
	status = InjExecuteShellcode(Process, IsWow64, Shellcode, ShellcodeParam);
	if (!NT_SUCCESS(status)) {
		LogError("InjExecuteShellcode failed with code %08x.", status);
